use bevy::prelude::*;
use bevy_round_ui::{prelude::{RoundUiMaterial, RoundUiBorder}, autosize::RoundUiAutosizeMaterial};

use crate::{entities::spaceship::{Health, Player}, AppState};
        ...
        p.spawn((
            MaterialNodeBundle {
                material: materials.add(RoundUiMaterial {
                    ...
                    border_radius: RoundUiBorder::all((PANEL_HEIGHT - PADDING * 2.0) / 2.0).into(), 
                    ..default()
                }), 
                style: Style {
...
impl Plugin for HealthBarPlugin {
    fn build(&self, app: &mut App) {
        app
            .add_systems(OnEnter(AppState::Running), health_bar_setup)
            .add_systems(Update, health_bar_update.run_if(in_state(AppState::Running)));
    }
}