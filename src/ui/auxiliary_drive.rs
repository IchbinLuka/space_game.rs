...
fn auxiliary_drive_setup(
    ...
    commands.spawn((
        TextBundle {
            text: Text::from_section(t!("auxiliary_drive", state = t!("state_off")), TextStyle {
                font_size: 60.0,
...
fn auxiliary_drive_update(
    ...
    for mut text in query.iter_mut() {
        for player in player_query.iter() {
            text.sections[0].value = t!("auxiliary_drive", state = if player.auxiliary_drive {
                t!("state_on")
            } else {
                t!("state_off")
