use bevy::{app::{App, Plugin, Update}, ecs::{component::Component, query::Changed, system::Query}, render::color::Color, text::Text, ui::{BackgroundColor, Interaction}};
...
fn hover_effect_text(
    ...
    for (hover_effect, interaction, mut text) in &mut query {
        let color = match *interaction {
            Interaction::None => {
                hover_effect.normal_color
            }
            Interaction::Hovered => {
                hover_effect.hover_color
            }
            _ => { continue; }
        };
